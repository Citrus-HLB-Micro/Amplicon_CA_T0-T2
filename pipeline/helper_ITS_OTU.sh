...
if [ -z $RUNNAME ]; then
	BASE=ECDRE_ITS_OTU_$(date +'%Y%m%d')
else
	BASE=$RUNNAME
fi
...
if [ ! -f $BASE.demux.fq.gz ]; then
	amptk illumina -i $INPUT --merge_method vsearch -f ITS1-F -r ITS2 --require_primer off -o $BASE --usearch usearch9  --rescue_forward on --primer_mismatch 2 -l 250 --cpus $CPU --cleanup
fi
